...
Inspired by the philosophy \textit{``People are at the center''} witnessed at DENSO --- where technology exists not to eliminate humans but to amplify their capabilities --- my research pursues a central question: \textit{How can AI and humans collaborate effectively in manufacturing environments?} From UWB-based warehouse tracking, to LLM-powered intelligent fault diagnosis at DENSO's factory floor, to my latest work on a \textbf{multi-agent reinforcement learning (MARL)} framework for AGV coordination --- my research trajectory aims to build \textbf{inclusive} Industry 4.0 systems where digital transformation upskills the workforce rather than displaces it.
...
\entry
  {\href{https://vjst.vn/cuoc-thi-hackathon-cua-denso-va-fpt-software-co-giai-thuong-200-trieu-dong-59481.html}{LLM-based Intelligent Fault Query System for Industrial Maintenance}}
  {Nov 2023 -- Jan 2024}
...
\vspace{0.5em}
\textbf{DENSO Factory Hacks 2023 --- Organizing Committee}
\newline
DENSO Manufacturing Vietnam $\times$ FPT Software
\newline
\textit{(Recommendation letter available upon request)}
